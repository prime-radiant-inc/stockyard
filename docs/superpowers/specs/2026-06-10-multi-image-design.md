...
- **CLI:** `stockyard image` command group: `ls` (REFERENCE / DIGEST / SIZE / CREATED table, answered by the *daemon* so it is truthful for remote daemons), `import`, and `rm` (which surface the daemon's errors verbatim). Redirect messages name real `container` CLI verbs (`load`/`pull`/`rm` — there is no `container image import`).
...
- **A registered image is:** a ZFS dataset `tank/stockyard/images/<dataset>` with an `@base` snapshot, plus a metadata row in the daemon's SQLite store: name (the OCI-style ref), dataset component, rootfs size in bytes (displayed human-readable), registered-at, optional kernel path (empty = shared `vmlinux.bin`). The dataset component is the sanitized name (allow `[a-zA-Z0-9._-]`, everything else → `-`), UNIQUE in the table; a sanitization collision fails the import legibly. FC-registered images carry no digest in v1 (`ImageInfo.digest` stays empty — hashing multi-GB rootfses on import isn't worth it yet).
- **Architecture:** the registry is a daemon-side component (SQLite + `zfs.Manager`) — NOT the Firecracker backend, which wraps only `firecracker.Client` and has no store access. The registry implements `vmbackend.ImageValidator` and `vmbackend.ImageLister`; the daemon consults the backend for apple-container and the registry for Firecracker. At VM-create time the daemon resolves name → snapshot path and threads it via a new `VMConfig.RootfsSnapshot` (empty = today's hardcoded `rootfs@base` fallback, preserving behavior), and threads the per-image kernel via the existing `VMConfig.KernelPath` empty-fallback.
- **The default image joins the registry.** At startup the daemon seeds a registered image named `default` from the configured `rootfs_path`, keeping its existing dataset component `rootfs` — zero ZFS migration for existing hosts. `stockyard run` without `--image` is equivalent to `--image default`. Only `default` self-heals at startup; other images whose datasets vanish fail legibly at use. `image rm default` is refused (it would be reseeded from config anyway).
...
- **Re-import replaces, scoped scorched-earth.** Importing over an existing name first destroys the tasks running that image *through the TaskManager* (proper teardown — the stored resolved `Task.Image` is the selector), then `zfs destroy -R`'s the dataset and replaces the row. This is today's deploy semantics scoped to one name, but orderly: no `pkill -9`, no daemon restart. `image rm` follows the same sequence minus the re-create. Generation-based replacement (old base drains as its clones exit) is a future refinement, not v1.
