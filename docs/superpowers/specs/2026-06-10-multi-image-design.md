...
- **Registration CLI:** `stockyard image import <name> --rootfs <path-on-host> [--kernel <path>]`, `stockyard image ls`, `stockyard image rm <name>`. These subcommands target the Firecracker backend; on macOS they fail with a pointer to `container image ls`/`rm`, since Apple's store is authoritative there.
